...
gpioMap_t teclas[] = {TEC1,TEC3,TEC4};
...
void tecla_led_init(void)
{
	...
	for(i = 0 ; i < N_TECLAS ; i++)
	{
		...
		tecla_led_config[i].queue_tec_pulsada = xQueueCreate( 1 , sizeof(TickType_t) );
		tecla_led_config[i].mutex 				=  xSemaphoreCreateMutex();

		// Gestion de errores de semaforos
		if( tecla_led_config[i].mutex == NULL)
		{
		...
		if( tecla_led_config[i].queue_tec_pulsada == NULL)
		{
			gpioWrite( LED_ERROR , ON );
